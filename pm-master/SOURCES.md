# 来源档案（SOURCES.md）

本文件记录 pm-master 的来源信息，以及产品经理模块的历次优化记录。仅作归档用途，不影响使用。

## 一、来源信息

| 项 | 内容 |
|---|---|
| 原作者 | SpaceZephyr（空格的键盘） |
| 原仓库 | https://github.com/SpaceZephyr/pm-skills |
| 原 Skill | `pm-master`（PM-Skill 2.0） |
| 原协议 | MIT License |
| Copy 时间 | 2026-09 |

## 二、参考与保留的机制

- **references**：无（pm-master 为总控，运行不需要额外参考资料）
- **保留机制**：分诊式路由（判断类 → 顾问团，产出类 → 执行链，链路类 → 编排）、预置工作流链路、成员名册与路由表

## 三、优化记录

### 3.1 收录与基础优化（2026-09，模块化收录时）

| 改动 | 原因 |
|---|---|
| 新增 README.md | 按 Skill-Bible 规范：用户打开 README 即知"是不是我要用的、我给什么、拿走什么" |
| 新增 SOURCES.md | 来源档案：保留原作者地址与参考机制，符合 MIT 协议要求 |

**收录背景**：Skill-Bible 采用模块化收录——一个职能模块尽可能覆盖完整工作链。产品经理模块由 `pm-master` 总控统一路由，上下游衔接见 SKILL.md 内「上下游衔接」章节。

### 3.2 pm-prd-writer 的标杆改动（2026-09，产品管理板块重构时完成）

pm-prd-writer 是本模块「参考模板」与「延展机制」两个最佳实践的源头，后续优化均对齐其做法：

| 改动 | 说明 |
|---|---|
| 「上下游衔接」改为通用描述 | 原版引用 `pm-master`/`pm-review-board`/`pm-tracking-spec-writer` 等当时未收录的 Skill，改为通用表述 + ≤10 行「交接摘要」输出规则，避免指向不存在的能力 |
| 新增「延展机制」章节 | 材料不足时参考同类成熟项目的产品机制，参考机制不照搬内容 |
| 恢复原版完整深度 | 从 24 行骨架恢复为 325 行完整版：阶段零需求体检、四阶段工作流（澄清→结构化→补漏→交付）、12 项质量检查、失败兜底策略 |
| 新增 `references/prd-template.md` | 完整五大章节 PRD 模板（原版 8.9KB 全量保留）——参考模板最佳实践的标杆来源 |
| description 微调 | 保持触发词完整，去掉对未收录 Skill 的依赖暗示 |

### 3.3 陆羽Skill 标准复检（2026-09，本轮）

按陆羽Skill（luyu-skill）标准对产品经理模块全部 22 个 Skill 复检后的优化。

**① 7 个执行型 Skill 补「输出参考模板」**

| Skill | 新增模板 | 说明 |
|---|---|---|
| pm-review-board | `references/review-report-template.md` | 评审结论报告 |
| pm-prioritization-engine | `references/priority-report-template.md` | 优先级排序报告 |
| pm-survey-designer | `references/survey-design-template.md` | 问卷设计文档 |
| pm-competitor-deconstructor | `references/competitor-analysis-template.md` | 竞品拆解报告 |
| pm-postmortem-writer | `references/postmortem-report-template.md` | 复盘报告 |
| pm-roadmap-planner | `references/roadmap-plan-template.md` | 产品路线图 |
| pm-tracking-spec-writer | `references/tracking-spec-template.md` | 埋点方案设计文档 |

- 模板由 SKILL.md 内嵌输出结构提取生成（保留原结构 + 写作指引），SKILL.md 输出章节加「📋 完整输出模板见 references/xxx-template.md」引用
- 收益：对齐 pm-prd-writer 的参考模板最佳实践，模板可独立复用/迭代；SKILL.md 保留原版深度（最小侵入，不删原内容）

**② 19 个 Skill 补「延展机制」章节**

- 统一补「延展机制」（pm-prd-writer 的更早已在 3.2 完成）：
  > 材料不足以把问题拆深时，去 GitHub 参考同类成熟项目的产品机制（如何拆解、如何区分类型、如何追问、如何验收），参考机制不照搬内容
- 收益：对齐 luyu-skill 规范"每个 Skill 内置延展机制"，Skill 具备自进化能力

**③ 边界检查结论**

- 原版 pm-skills 的「兜底策略」章节已覆盖"不编造、降级"等诚实边界，未重复添加（避免冗余）
- 原版「上下游衔接」引用的兄弟 Skill 全部存在于当前模块（22 个），引用有效，未改动

**④ 未优化的部分（说明）**

- pm-prd-writer：本轮未再改动（改动已在 3.2 完成，已是参考模板与延展机制的标杆）
- pm-master：总控职责不变，仅记录本条优化档案
- 4 个带 HTML 资产模板的 Skill（analytics/experiment/roadmap/tracking）：已有 `assets/*.html` 运行模板，不再加 Markdown 模板

### 3.4 优化原则

按陆羽Skill"有优化空间就优化、没有大优化点就不为优化而优化"：只做**低风险高价值**的结构性补齐（模板 + 延展机制），保留原版全部判断深度，不重写成熟内容。
