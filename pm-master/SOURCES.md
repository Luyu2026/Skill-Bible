...
按陆羽Skill（luyu-skill）标准对产品经理模块全部 22 个 Skill（含 pm-prd-writer）复检后，优化如下：

### 0. pm-prd-writer 的既有改动（早于本轮，产品管理板块重构时完成）

> 图片确认：PRD skill 的「上下游衔接」与「延展机制」章节即为此轮改动成果。
...
- **定位**：pm-prd-writer 是本模块「参考模板」与「延展机制」两个最佳实践的源头，本轮其他执行型 Skill 的模板化即对齐它的做法
...
- 19 个 Skill 统一补「延展机制」（pm-prd-writer 的延展机制更早于产品管理重构时已加，见第 0 节）：
...
- pm-prd-writer：本轮未再改动（它的改动在更早的产品管理重构已完成，见第 0 节；已是参考模板与延展机制的标杆）
