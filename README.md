...
拿求职来说，它从来不是一次性任务：先判断岗位值不值得投，再改简历；突然收到面试，要准备项目和题目；面完以后，还要把卡住的问题练成下次能说的答案。

这些环节看起来连在一起，实际考验的是不同能力。直接做一个大而全的 Skill，往往每一步都只做到“差不多”；先把每个环节单独做深，再由总控把它们接起来，结果会更稳。

所以求职工作流是：

`岗位匹配 → 简历定制 → 面试准备 → 多轮面试准备 → 面试复盘`

你不需要记住这串名字。只要告诉求职全流程 Skill：你现在遇到了什么，它会判断当前最该推进哪一步，再调度对应的深度能力。
