...
将下面的 `interview-prep-brief` 替换为你想要的目录名。这个方式只拉取对应 Skill，不需要下载整个仓库。

```bash
git clone --depth 1 --filter=blob:none --sparse https://github.com/Luyu2026/Skill-Bible.git
cd Skill-Bible
git sparse-checkout set interview-prep-brief

# Claude Code 示例：复制到默认 Skill 目录
cp -R interview-prep-brief ~/.claude/skills/
```

Codex CLI 用户将最后一行改为：

```bash
cp -R interview-prep-brief ~/.codex/skills/
```
