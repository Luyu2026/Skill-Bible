...
| 信号 | 危险表现 | 处理 |
|------|---------|------|
| 需求来源 | 只有"老板说/客户提了一嘴/竞品有"，没有任何用户证据 | 提示风险，建议先做用户访谈/调研验证真伪，再算用户价值 |
...
本 Skill 独立可用，也常作为一条产品工作链的一环。

- **上游**：需求真伪与价值判断（如访谈、调研）、竞品差异化结论、数据洞察
- **下游**：拿 PRD 去模拟评审、把统计需求章节转成埋点方案、对需灰度验证的功能设计实验

交接规则：链路模式下，完成后输出一段「交接摘要」（≤10 行：本步结论 + 下一步所需输入），供下一个 Skill 直接使用。

## 延展机制

默认使用用户提供的材料、已有知识库和本 Skill 内置能力。当材料不足以把问题拆深，或用户明确要求进一步研究时，可以去 GitHub 参考同类成熟项目的**产品机制**（如何按场景拆 PRD 结构、如何区分必答与可选澄清项、如何做自动补漏、如何给待确认项排序），参考的是机制，不是照搬内容。发现可复用的检查项后，补入本 Skill 下一版。
