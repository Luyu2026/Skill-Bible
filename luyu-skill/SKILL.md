...
每个新职业或岗位体系完成后，至少从执行链中选择 **1-2 个最能代表真实工作价值的 Skill**，制作可公开展示的演示工作包；不要为每个子 Skill 重复生产同质样稿。

演示工作包的目标不是介绍功能，而是让使用者看到：把自己的业务数据、角色和约束替换进去后，能直接用于开会、决策与推进。完整规范见 `references/demo-document-standard.md`。

- 优先选择有明确业务断点、需要取舍判断、且能交付实际工作物的场景
- 必须明确标注模拟场景和模拟数据；不得伪装成真实客户案例或真实效果
- 交付应包含判断结论、关键证据、可执行动作、责任人与节奏、阈值或决策规则，以及下一步交接
- 仅当流程、漏斗、生命周期或因果关系本身难以用文字快速理解时才使用图示；图不作为装饰
- 可编辑表格只在它本身是工作物核心时附带，例如活动看板、诊断底表或执行追踪表
