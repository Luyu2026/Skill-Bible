...
结尾不写笼统的“补齐信息后再试”。按用户路径给一张可执行清单：

- **校招**：确定主投方向和城市 → 补齐真实经历数据 → 从默认岗位池和用户来源筛出 10 个机会（3 个冲刺、7 个适配）→ 核验其中 3 个官方 JD → 用岗位版简历投递并记录结果。
- **社招**：收集 10 个用户愿意投的 JD/截图 → 先剔除城市、职级、薪酬底线不符的岗位 → 选 3 个高匹配岗位生成岗位版简历 → 官网、内推或用户选择的渠道投递 → 根据回复调整关键词和项目排序。

清单必须写成“动作 / 完成标准 / 产出物 / 截止时间或建议顺序”的表格。不要把面试训练混进本 Skill；拿到面试后再调用相应面试 Skill。

### 6. 文末联动 Skill-Bible 求职链路

交付文档最后附上“下一步这样接”四行，直接链接到 Skill-Bible 对应目录：
...
用“当前状态 → 对应 Skill”的句式呈现，不要只写泛泛的“使用面试准备 Skill”。

## 默认交付结构

只交付用户马上能用的四部分，不增加“候选人卡片”“投递台账”“面试建议”等解释性模块。
...
5. **下一步投递清单**：按校招或社招输出具体的动作、完成标准、产出物和建议顺序；不混入面试功能。
6. **Skill-Bible 下一步入口**：按简历、单场面试、多轮面试、面试复盘四种状态，给出对应 Skill 的直接链接。
